...
<div class="space-y-8">
    ...
    <div class="glass-effect rounded-2xl p-8 border border-[#333]">
        ...
        <div class="space-y-4">
            @foreach($work as $row)
                ...
                <div class="flex justify-between py-3 border-b border-[#333] last:border-b-0">
                    <span>
                        @if($from === $to)
                            {{ $from }}
                        @else
                    ...
                    @else
                        <span class="font-medium">{{ $row['time'] }} Uhr</span>
                    @endif
                </div>
            @endforeach
        </div>
    ...
    <div class="glass-effect rounded-2xl p-8 border border-[#333] bg-gradient-to-br from-[#1E1E1E] to-[#2D2D2D]">
        ...
        <a href="tel:{{ $phone }}"
           class="block w-full bg-transparent border-2 border-[#BFBFBF] hover:bg-[#BFBFBF] hover:text-[#1E1E1E] text-white py-3 rounded-lg font-medium transition-all mt-6 text-center">
